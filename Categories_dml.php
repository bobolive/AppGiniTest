...
function Categories_insert(&$error_message = '') {
	...
	if(function_exists('Categories_after_insert')) {
		$res = sql("SELECT * FROM `Categories` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
...
function Categories_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Projects` WHERE `Category`='" . makeSafe($id[0]) . "'", $eo);
...
function Categories_update(&$selected_id, &$error_message = '') {
	...
	if(function_exists('Categories_after_update')) {
		$res = sql("SELECT * FROM `Categories` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
...
function Categories_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$res = sql("SELECT * FROM `Categories` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Categorie details', $templateCode);
