...
function Clients_insert(&$error_message = '') {
	...
	$data = [
		'Clients_name' => Request::val('Clients_name', ''),
	];
...
function Clients_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#Clients_name').replaceWith('<div class=\"form-control-static\" id=\"Clients_name\">' + (jQuery('#Clients_name').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(Clients_name)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(Clients_name)%%>', safe_html($urow['Clients_name']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(Clients_name)%%>', html_attr($row['Clients_name']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Clients_name)%%>', urlencode($urow['Clients_name']), $templateCode);
	} else {
		$templateCode = str_replace('<%%VALUE(id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(id)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(Clients_name)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Clients_name)%%>', urlencode(''), $templateCode);
	}
