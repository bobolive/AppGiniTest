<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'Projects' => "`Projects`.`id` as 'id', `Projects`.`Projects_name` as 'Projects_name', `Projects`.`Description` as 'Description', IF(    CHAR_LENGTH(`Categories1`.`Category_name`), CONCAT_WS('',   `Categories1`.`Category_name`), '') as 'Category', `Projects`.`ProjectTags` as 'ProjectTags', `Projects`.`Image_1` as 'Image_1', `Projects`.`Image_Uploaded` as 'Image_Uploaded', `Projects`.`Geo_place` as 'Geo_place'",
			'Tags' => "`Tags`.`id` as 'id', `Tags`.`Tag` as 'Tag'",
			'Categories' => "`Categories`.`id` as 'id', `Categories`.`Category_name` as 'Category_name', `Categories`.`Description` as 'Description'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'Projects' => "`Projects` LEFT JOIN `Categories` as Categories1 ON `Categories1`.`id`=`Projects`.`Category` ",
			'Tags' => "`Tags` ",
